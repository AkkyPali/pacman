        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if (nextx, nexty) in self.corners:
                    if (nextx, nexty) not in visited_corners:
...
def cornersHeuristic(state, problem):
    ...
    coords, visited_corners = state
    not_visited_corners = [x for x in corners if x not in visited_corners]

    h_distance = 0
    while len(not_visited_corners) > 0:
        distanceCorner = []
        for corner in not_visited_corners:
            distance = util.manhattanDistance(coords, corner)
            distanceCorner.append((distance, corner))
        curr_distance, curr_corner = min(distanceCorner)
        h_distance += curr_distance
        coords = curr_corner
        not_visited_corners.remove(curr_corner)
    return h_distance
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        if (position, food) not in problem.heuristicInfo.keys():
    ...
    for food_s in foodGrid.asList():
        for food_g in foodGrid.asList():
            if (food_s, food_g) not in problem.heuristicInfo.keys():
                problem.heuristicInfo[(food_s, food_g)] = mazeDistance(food_s, food_g, problem.startingGameState)
            distances_food.append(problem.heuristicInfo[(food_s, food_g)])
